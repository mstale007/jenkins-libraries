...
def sendAttachment(Map args = [attachmentLink: "target/site/", issue: ""]) {
    ...
    if(isUnix()) {
        sh(script: "zip Build" + env.BUILD_NUMBER + "-" + env.BRANCH_NAME + "-BDD.zip " + link + "/cucumber-html-reports_fb242bb7-17b2-346f-b0a4-d7a3b25b65b4")
        sh(script: "curl -s -i -X POST \"" + env.JIRA_BOARD + "/issue/"+issue_ID+"/attachments\" --header \"Authorization:" + env.AUTH_TOKEN + "\" --header \"X-Atlassian-Token:no-check\" --form \"file=@Build-" + env.BUILD_NUMBER + "-" + env.BRANCH_NAME + "-BDD.zip\"")
    }
    else {
        bat(script: "powershell Compress-Archive " + link + "/cucumber-html-reports_fb242bb7-17b2-346f-b0a4-d7a3b25b65b4 Build" + env.BUILD_NUMBER + "-" + env.BRANCH_NAME + "-BDD.zip")
        bat(script: "curl -s -i -X POST \"" + env.JIRA_BOARD + "/issue/"+issue_ID+"/attachments\" --header \"Authorization:" + env.AUTH_TOKEN + "\" --header \"X-Atlassian-Token:no-check\" --form \"file=@Build" + env.BUILD_NUMBER + "-" + env.BRANCH_NAME + "-BDD.zip\"")
    }
}
